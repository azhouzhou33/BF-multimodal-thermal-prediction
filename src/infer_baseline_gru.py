#!/usr/bin/env python3
"""
Baseline GRU model inference script
Load trained baseline GRU model for blast furnace thermal state prediction

Usage:
python src/infer_baseline_gru.py --dataset TuyereData/DataSetTrain_halfHourly --model-path logs/training_GRU/1.5/val_loss_0.0154_epoch_40.h5
"""
...
# Set to use CPU (modify to corresponding GPU number if GPU is needed)
os.environ['CUDA_VISIBLE_DEVICES'] = '-1'


def parse_arguments():
    """Parse command line arguments"""
    parser = argparse.ArgumentParser(description="Baseline GRU model inference")
    parser.add_argument("-d", "--dataset", type=str, required=True,
                       help="Input dataset path")
    parser.add_argument("-m", "--model-path", type=str, 
                       default="Save_model/baseline_gru_model.h5",
    ...
    parser.add_argument("--scaler-path", type=str, default="scalers/baseline_gru_scaler.pkl",
                       help="Path to saved scaler file (default: scalers/baseline_gru_scaler.pkl)")
...
def main():
    ...
    print(f"Test prediction length: {len(test_pred)}")
    
    # Validation set predictions
    val_preds = model.predict(val_attr_x)
    ...
    plot_predictions(train_pred_original, train_y_original, "Training Set",
                    os.path.join(args.output_dir, "baseline_train_predictions.png"))
    
    # Plot validation set prediction results
    plot_predictions(val_pred_original, val_y_original, "Validation Set",
                    os.path.join(args.output_dir, "baseline_val_predictions.png"))
    
    # Plot test set prediction results
    plot_predictions(test_pred_original, test_y_original, "Test Set",
                    os.path.join(args.output_dir, "baseline_test_predictions.png"))
    
    # Plot test set scatter plot
    plot_scatter(test_pred_original, test_y_original, 
                xlim=(1450, 1550), ylim=(1450, 1550),
                save_path=os.path.join(args.output_dir, "baseline_scatter_plot.png"))
    ...
    plt.title('Actual vs Predicted Values (Baseline GRU Model)')
    plt.xlabel('Actual Values')
    plt.ylabel('Predicted Values')
    plt.legend()
    plt.grid(True, alpha=0.3)
    
    combined_scatter_path = os.path.join(args.output_dir, "baseline_combined_scatter.png")
    ...
    if args.save_predictions:
        save_predictions(test_pred_original, test_y_original,
                        os.path.join(args.output_dir, "baseline_test_predictions.csv"))
        save_predictions(val_pred_original, val_y_original,
                        os.path.join(args.output_dir, "baseline_val_predictions.csv"))
